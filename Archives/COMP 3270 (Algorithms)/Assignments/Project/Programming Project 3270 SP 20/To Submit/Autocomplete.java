...
public class Autocomplete {
   ...
   /**
    ...
    public static class BinarySearchAutocomplete implements Autocompletor {
        ...
        public double weightOf(String term) {
           for (Term t: myTerms) {
              if (t.getWord().equals(term)) return t.getWeight();
           }
           return 0.0;
        }
    }
    ...
    public static class TrieAutocomplete implements Autocompletor {
        ...
        private void add(String word, double weight) {
           ...
           node.mySubtreeMaxWeight = Math.max(weight, node.mySubtreeMaxWeight);
           for (int i = 0; i < word.length(); i++) {
              ...
              if (node.children.containsKey(curr)) {
                 ...
                 node.mySubtreeMaxWeight = Math.max(weight, node.mySubtreeMaxWeight);
                 if (i == word.length()-1) {
                    node.setWeight(Math.max(weight, node.getWeight()));
                    node.setWord(word);
                 }
              } else {
                 node.children.put(curr, new Node(curr, node, weight));
                 node = node.getChild(curr);
                 if (i == word.length()-1) {
       ...
       public Iterable<String> topMatches(String prefix, int k) {
          ...
          PriorityQueue<Node> q = new PriorityQueue<>((x,y) ->
                  Double.compare(y.mySubtreeMaxWeight,x.mySubtreeMaxWeight));
          ...
          while(!q.isEmpty() && count < k) {
             Node curr = q.poll();
             if (curr.getWeight() == curr.mySubtreeMaxWeight) {
                count++;
        ...
        public String topMatch(String prefix) {
            ...
            for (int i = 0; i < prefix.length(); i++) {
               ...
               if (!node.children.containsKey(curr)) return "";
               node = node.getChild(curr);
            }
            
            while (node.mySubtreeMaxWeight != node.getWeight()) {
               for (char temp: node.children.keySet()) {
                  if (node.getChild(temp) != null && node.getChild(temp).mySubtreeMaxWeight == node.mySubtreeMaxWeight) {
        ...
        public double weightOf(String term) {
            ...
            for (int i = 0; i < term.length(); i++) {
               if (!node.children.containsKey(term.charAt(i)))  return 0.0;
               node = node.getChild(term.charAt(i));
            }
